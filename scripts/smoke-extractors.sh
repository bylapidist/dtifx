...
PACKAGE_ROOT="${REPO_ROOT}/packages/extractors"

if [[ ! -d "${PACKAGE_ROOT}" ]]; then
  echo "@dtifx/extractors package directory not found" >&2
  exit 1
fi

echo "Building @dtifx/core, @dtifx/cli, and @dtifx/extractors packages for smoke test" >&2
pnpm exec nx run-many -t build --projects core,cli,extractors --output-style=static
...
echo "Running extractor smoke test with package artifact ${PKG_NAME}" >&2
if [[ -n "${CORE_PKG_PATH}" ]]; then
