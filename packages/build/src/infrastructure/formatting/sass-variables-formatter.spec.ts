...
import { createSassVariablesFormatterFactory } from './sass-variables-formatter.js';
...
describe('createSassVariablesFormatterFactory', () => {
  it('emits Sass variables for supported token types', async () => {
    const factory = createSassVariablesFormatterFactory();
    const entry = { name: 'sass.variables', output: {} };
  ...
  it('respects custom filename and prefix options', async () => {
    ...
    const entry = {
      name: 'sass.variables',
      output: {},
      options: { filename: 'tokens/_variables.scss', prefix: 'ds' },
    };
    const context = { config: {} as BuildConfig };
    const definition = factory.create(entry, context);
  ...
  it('validates option types', () => {
    ...
    expect(() => factory.create(entry, context)).toThrow(
      'Formatter "sass.variables" filename must be a non-empty string.',
    );
  });
});
