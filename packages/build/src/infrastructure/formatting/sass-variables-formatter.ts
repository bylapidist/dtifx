...
const FORMATTER_NAME = 'sass.variables';
const DEFAULT_FILENAME = 'tokens.scss';
const OPTION_KIND: ConfigOptionKind = 'formatter';
const OPTION_KEYS = new Set(['filename', 'prefix']);

interface SassVariablesFormatterOptions {
  readonly filename: string;
  readonly prefix?: string;
}

/**
 * Creates the formatter definition factory responsible for emitting Sass variables.
 * @returns {FormatterDefinitionFactory} The Sass variables formatter factory.
 */
export function createSassVariablesFormatterFactory(): FormatterDefinitionFactory {
  return {
    name: FORMATTER_NAME,
    create(entry) {
      const options = parseOptions(entry.options, entry.name);
      return createSassVariablesFormatterDefinition(options);
    },
...
function parseOptions(
  ...
  name: string,
): SassVariablesFormatterOptions {
  if (rawOptions === undefined) {
    return { filename: DEFAULT_FILENAME } satisfies SassVariablesFormatterOptions;
  }
  ...
  return {
    filename,
    ...(prefix ? { prefix } : {}),
  } satisfies SassVariablesFormatterOptions;
}

function createSassVariablesFormatterDefinition(
  options: SassVariablesFormatterOptions,
): FormatterDefinition {
  return {
    name: FORMATTER_NAME,
    ...
    run: async ({ tokens }) => {
      ...
      const contents = formatSassVariables(declarations);
...
function normaliseFilename(value: string): string {
  ...
  if (trimmed.length === 0) {
    throw new TypeError('Formatter "sass.variables" filename must be a non-empty string.');
  }
  return trimmed;
}
...
function normalisePrefix(value: string): string {
  ...
  if (trimmed.length === 0) {
    throw new TypeError(
      'Formatter "sass.variables" prefix must be a non-empty string when provided.',
    );
  }
  return trimmed;
}

function formatSassVariables(declarations: readonly WebVariableDeclaration[]): string {
